...
// Resolve the GitHub org/owner. An explicit --org always wins; otherwise we
// auto-detect the authenticated GitHub user via the gh CLI. Returns null when
// gh is missing or unauthenticated.
function resolveOrg(explicitOrg) {
...
const resolvedOrg = resolveOrg(flags.org);
// The org is genuinely required for the connect and rename paths. Discover and
// dry-run can proceed without it (dry-run just previews with a placeholder).
const orgRequired = !DISCOVER && !DRY_RUN;
if (!resolvedOrg && orgRequired) {
  console.error(
    "Could not determine GitHub org. Pass --org <name> or run `gh auth login`."
  );
  process.exit(1);
}
const ORG = resolvedOrg ?? "your-org";
...
// Persistent browser profile (stores Lovable login cookies). Resolved relative
// to this script — not the current working directory — so the cookie-bearing
// profile always lands beside connect.mjs and stays covered by .gitignore,
// regardless of where the tool is launched from.
const PROFILE_PATH = new URL("browser-data", import.meta.url).pathname;
